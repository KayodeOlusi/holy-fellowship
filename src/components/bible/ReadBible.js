...
const ReadBilbe = () => {
  ...
  const holdBook = (e) => {
    ...
    fetch(
      `https://api.scripture.api.bible/v1/bibles/de4e12af7f28f599-01/books/${e.target.value}/chapters`,
      {
        method: "GET",
        headers: {
        ...
        <div className="details">
          <FormControl required xs={{ m: 1, minWidth: 60 }}>
            ...
            >
              {books?.map((book) => {
                return (
                  <MenuItem value={book.id} id={book.id} key={book.id}>
                    {book.name}
                  </MenuItem>
                );
              })}
          ...
          <FormControl required xs={{ m: 1, minWidth: 60 }} className="form">
            ...
            >
              {chapter?.map((chapt) => {
                return (
                  <MenuItem value={chapt.number} id={chapt.id} key={chapt.id}>
                    {chapt.number}
                  </MenuItem>
                );
              })}
